...
fn digiroot() {
    ...
    assert_eq!(run_file_with_input(DIGIROOT, b"99999999").unwrap().trim(), "9");
...
#[test]
fn quines() {
    for quine_file in ["programs/kquine.bf", "programs/kquine2.bf", "programs/kquine3.bf"] {
        assert_eq!(run_file(quine_file).unwrap(), std::fs::read_to_string(quine_file).unwrap());
    }
}
